...
if length(findstr(cadena,'pertoire'))>0
	...
	try
		...
		display('french_strings.m pas trouvé')
		english_strings
	end
...
if (pantalla==0)
	...
	buts0 = gtk(in,out,["gtk_button_new_with_label ",string002]);
...
if (pantalla==1) || ((pantalla==0) && (strcmp(tipo,'SON')))
	...
	lbls10split = gtk(in,out,["gtk_label_new ",string033]);gtk(in,out,["gtk_misc_set_alignment ",lbls10split,"1 0.5"]);
	...
	s10_blend = gtk(in,out,["gtk_check_button_new_with_label ",string034," 1"]);
	hseps2bs3 = gtk(in,out,"gtk_hseparator_new");
...
if (pantalla==1) && (strcmp(tipo,'SON')==0)
	...
	buts9_joinall = gtk(in,out,["gtk_button_new_with_label ",string036]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%		
		if strcmp(event, buts2_refresh )
			...
			try	
				...
				tamfile = str2num(gtk(in,out,["gtk_entry_get_text ", sizes2]));
				paso = str2num(gtk(in,out,["gtk_entry_get_text ", steps2]));
				cadena=sprintf('%d _relleno %s',calcular_num_rellenos(ficherokml,tamfile,paso),string042);
		...
		if strcmp(event, buts3_createprofile )
			...
			try
				...
				if mod(smooth_factor1,2)==1
					...
					informa_anyade(in,out,advb,string043);
				end
			catch
				informa_anyade(in,out,advb,string041);
			end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(event, buts3_consolidate)
			...
			%try
				...
				if isempty(who('lbls10_distancia'))~=1
					inicializa_s10(in,out,buts10_createsplit,buts10_splittrack,buts10_createterrain,lbls10_distancia);
				end
				informa_anyade(in,out,advb,string040);
			%catch
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(event, buts1)
			...
			try
				...
				gtk(in,out,["gtk_progress_bar_set_fraction ",progress_bar.id,sprintf('%.1f',1)]);gtk(in,out,"gtk_server_callback update");
				informa_anyade(in,out,advb,string040);
				if (strcmp(tipo,'FATHER')==1)
					informa_anyade(in,out,advb2,string051);
				end
			catch
				gtk(in,out,["gtk_progress_bar_set_fraction ",progress_bar.id,sprintf('%.1f',0)]);gtk(in,out,"gtk_server_callback update");
				informa_anyade(in,out,advb,string041);
...
global string047;
    ...
    if (exist('..\agr')==7) || (exist('..\lidar')==7) || (numero_padres~=0) %No usamos s2_elevation
				...
				if (exist('..\agr')==7)
					gtk(in,out,["gtk_label_set_text ",lbls2limits,string044])
				else
				...
				if (numero_ficheros>0)
					gtk(in,out,["gtk_label_set_text ",lbls2limits,'"',ficherokml,string046])
				else
				...
				comprueba(in,out,['dir ',folder,'\salida\grid*_relleno.kml /b'],buts2_readgrid);
				comprueba(in,out,['dir ',folder,'\salida\lamalla.mat /b'],buts2_plotlamalla);
		end
end
